...
#packrat::snapshot(ignore.stale=TRUE) # updated the libraries in the environment manually loaded in R
#dpackrat::restore() # requires R to use the environment's package versions
     ...
     dashboardBody(
       tabItems(
         ...
         tabItem(
           tabName = "words",
           fluidRow(
             # First row with selectInput in the left column and radioButtons in the right column
             column(
               width = 6, 
               selectInput(
                 "typeSelectWords", 
           ...
           fluidRow(
             # Second row with images in the left and right columns
             column(
               width = 6,
               imageOutput("wordCloudImage")  # This will display the word cloud image
             ),
