...
#######################   
  ...
  if (!is.null(belong_data) && nrow(belong_data) > 0) {
      ...
      belong_data <- belong_data %>%
        ...
        mutate(fraction = as.numeric(freq) / sum(as.numeric(freq))) %>%  # Ensure numeric column
        as.data.frame()  # Ensure it is not a named vector
      ...
      # Define Color Scale
      color_scale_b <- unname(c("#A1D296", "#899A75", "#516841","#518241" ,"#567344", "#9FD430", "#7BAF40", "#7BAE28", "#154733", "#004225")) # Expanded for donuts b
...
#######################  
  ...
  output$wordNetImage <- renderUI({
    ...
    # **Alt Text Generation**
    alt_text <- if (file.exists(file.path("www", full_image_path_net))) {
      paste(
    ...
    div(
      ...
      img(src = if (file.exists(file.path("www", full_image_path_net))) full_image_path_net else no_data_image_path, 
...
#######################   
  ...
  output$emotionImage <- renderUI({
    ...
    img_src <- if (file.exists(file.path("www", full_image_path))) {
      full_image_path
    } else {
      no_data_image_path
    }
  ...
  # observe({
  #   req(output$emotionImage)  # Ensure the element exists before adding the popover
  #   
  #   addPopover(session, id = "emotionImage",
  #              title = "Campus Emotions",
